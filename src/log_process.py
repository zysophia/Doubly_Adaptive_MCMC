...
if __name__ == "__main__":
    ...
    # print(df)  
    # df.to_csv("../data/compare_complexity.csv", index=False)


    datalist = []
    f = open("../logs/isingoutput.log", 'r')
    for line in f.readlines():
        ss = line.split(",")
    ...
    df = pd.DataFrame(datalist, columns=["chain", "n", "beta_max", "epsilon", \
        "algorithm", "sample_complexity", "TPAsteps", "true_Q", "estimate_Q", "error"])
    print(df)  
    df.to_csv("../data/isingcompare_complexity.csv", index=False)
